...
if (isset($_POST['v_post_date'])) {
    $v_post_date = $_POST['v_post_date'];
} else {
    $v_post_date = '';
}
if (isset($_POST['v_detail'])) {
    $v_detail = $_POST['v_detail'];
} else {
    $v_detail = '';
}
if (isset($_POST['v_update_by'])) {
    $v_update_by = $_POST['v_update_by'];
} else {
    $v_update_by = 0;
}
if (isset($_POST['v_updatedt'])) {
    $v_updatedt = $_POST['v_updatedt'];
} else {
    $v_updatedt = '';
}
if (isset($_POST['unit'])) {
...
//    $v_video = '';
//}
$v_video = $_POST['v_path'];


if ($action == 'register') {

    $sql = "INSERT INTO `videos` ( `v_title`, `v_user`, `v_video`, `v_cover`, `v_post_date`, `v_status`,  `v_detail`,`category_id`) "
            . "VALUES ( '$v_title', '$v_user', '$v_video', '$v_cover', '$v_post_date', '0',  '$v_detail',  '$unit')";
...
if ($action == 'update' && $v_id > 0) {
    ...
    $sql = "update videos set  v_title='" . $v_title . "',category_id=\"$unit\" ,v_detail ='" . $v_detail . "'$sql_img  $sql_v  where v_id='" . $v_id . "'";
